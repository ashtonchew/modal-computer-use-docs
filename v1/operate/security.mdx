...
Use Connect ingress when your policy requires all daemon traffic to use Modal Connect. Raw and attested tunnel modes do not trust a client-supplied verified-user header.
...
Boolean `True` maps to `"control"`. It does not map to view-only.
...
Use the [view-only example](https://github.com/ashtonchew/modal-computer-use/blob/main/examples/novnc_view_only.py) to create a desktop with view-only noVNC access. The example reports only whether a URL exists. It does not print the URL.
...
Set Sandbox lifetime, idle policy, action budgets, screenshot budgets, artifact quotas, and recording limits. These controls bound resource use. They do not replace authentication.
...
The OpenAI and Anthropic adapters normalize actions. They are not policy engines.
...
Report a product vulnerability through the repository [security policy](https://github.com/ashtonchew/modal-computer-use/blob/main/SECURITY.md).
